...
def main(args):
    ...
    dataset = SeqTaggingClsDataset(data, vocab, tag2idx, args.max_len,train=True)
    ...
    for idx, batch in enumerate(test_dataloader):
        ...
        gt = batch['tags'].reshape(-1)
        index = 0
        basis = max(batch['seq_len'])
        for id, i in enumerate(batch['seq_len']):
            predic.append([dataset.idx2label(i) for i in (pred[index:index+i].tolist())])
            groundt.append([dataset.idx2label(i) for i in (gt[index:index+i].tolist())])
            index += basis


    print(classification_report(predic, groundt, scheme=IOB2, mode='strict'))
